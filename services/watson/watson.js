...
audioToWav = async (audioLink) => {
  if (audioLink && !audioLink.includes('http')) { // local file
...
exports.analyzeTone = async (text) => {
  const resp = await toneAnalyzer.tone({ toneInput: text, contentType: 'text/plain' });
  if (!resp || !resp.result) {
...
exports.transcribe = async (audioLink) => {
  ...
  const params = { audio: fs.createReadStream(audioPath), contentType: 'audio/wav' };
...
exports.translate = async (text, sourceLang = 'en', toLang = 'en') => {
  const resp = await languageTranslator.translate({ text: [text], source: sourceLang, target: toLang });
  if (!resp || !resp.result || !resp.result.translations) {
    throw new Error(`Could not translate from ${sourceLang} to ${toLang} string: ${text}`);
  }
...
exports.identifyLanguage = async (text, returnTop = false) => {
  ...
  console.log('identifyLanguage# language confidences:', resp.result.languages);
  if (returnTop) {
    return resp.result.languages.reduce( (a, b) => (a.confidence > b.confidence) ? a : b );
  }
